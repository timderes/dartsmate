...
describe("Multiplayer Game Logic Synchronization", () => {
  it("should initialize identical states for Host and Guest from the same Match Data", () => {
    ...
    const hostState = gameReducer({} as GameState, {
      type: "INIT_GAME",
      payload: matchData,
    });

    // Guest initializes from the received INIT_GAME broadcast
    const guestState = gameReducer({} as GameState, {
      type: "INIT_GAME",
      payload: matchData,
    });
  ...
  it("should remain perfectly synchronized after a sequence of throws and turn switches", () => {
    ...
    const initialState = gameReducer({} as GameState, {
      type: "INIT_GAME",
      payload: matchData,
    });
  ...
  it("should handle UNDO_THROW correctly across clients", () => {
    const matchData = createInitialMatchData();
    let state = gameReducer({} as GameState, {
      type: "INIT_GAME",
      payload: matchData,
    });
    ...
    const expectedEmptyRoundState = gameReducer({} as GameState, {
      type: "INIT_GAME",
      payload: matchData,
    });
